...
class GUI_select_robot:
	def __init__(self, master):
		...
		self.button_robot_1 = tk.Button(self.frame, image=self.img_robot_1, command = lambda *args:[player.change_settings(player.player_info, ['name'], ['192.666.1.38']), Connect(player.player_info['robot'], PORT, turtle_conn) ,self.new_window()])
		self.button_robot_2 = tk.Button(self.frame, image=self.img_robot_2, command = lambda *args:[player.change_settings(player.player_info, ['robot'],['192.666.1.39']), Connect(player.player_info['robot'], PORT, turtle_conn), self.new_window()])
